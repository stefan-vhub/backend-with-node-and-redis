...
router.get("/:restaurantId/weather", checkRestaurantExists, async (req: Request<{restaurantId: string}>, res, next) => {
    ...
    try {
        const clinet = await initializeRedisClient();
        const watherKey = weatherKeyById(restaurantId);
        const cachedWeather = await clinet.get(watherKey);
        if (cachedWeather) {
            console.log("Cache Hit");
            return successResponse(res, JSON.parse(cachedWeather));
        }
        const restaurantKey = restaurantKeyById(restaurantId);
        const coords = await clinet.hGet(restaurantKey, "location");
        if (!coords) {
        ...
        if (apiResponse.status === 200) {
            ...
            await clinet.set(watherKey, JSON.stringify(json), {EX: 60 * 60});
            return successResponse(res, json);
        }
        return errorResponse(res, 500, "Couldnt fetch wather info");
    } catch (error) {
        next(error);
    }
});
...
router.post("/:restaurantId/reviews", checkRestaurantExists, validate(ReviewSchema), async (req: Request<{ restaurantId: string }>, res, next) => {
    ...
    try {
        ...
        await Promise.all([
            ...
            client.hSet(restaurantKey, "avgStars", avarageRating)
        ])
